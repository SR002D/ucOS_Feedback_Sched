    ...
    for (i = 0;i <= OS_LOWEST_PRIO;i++) {
        ...
        if (OSTCBPrioTbl[i] != (OS_TCB*)0) {
            ...
            if (i % 2 == 0) {
                ...
                // 非周期任务
                ptcb = Sched_EDF(OSTCBPrioTbl[i]);
                //ptcb = Sched_FIFO(OSTCBPrioTbl[i]);
            }
        }
...
OS_TCB* Sched_RMS(OS_TCB* tcblist) {
	OS_TCB *ptcb;
...
OS_TCB* Sched_EDF(OS_TCB* tcblist) {
	OS_TCB *ptcb;
	ptcb = tcblist;   
	INT32U min_ddl;
	tcb_ext_info* task_info;
...
static void TaskAdd(OS_TCB* ptcb) {
    tcb_ext_info* task_info;
    ...
    // 添加到OSTCBPrioTbl指定优先级的链表中
    ptcb->OSTCBNext = OSTCBPrioTbl[ptcb->OSTCBPrio];
    ptcb->OSTCBPrev = (OS_TCB*)0;
    if (OSTCBPrioTbl[ptcb->OSTCBPrio] != (OS_TCB*)0) {
        OSTCBPrioTbl[ptcb->OSTCBPrio]->OSTCBPrev = ptcb;
    }
